...
class Database:
    def __init__(self, db_path="pixiv_vault.db"):
        try:
            self.conn = sqlite3.connect(db_path, check_same_thread=False, autocommit=True)
        except TypeError:
            self.conn = sqlite3.connect(db_path, check_same_thread=False, isolation_level=None)
        self.conn.row_factory = sqlite3.Row
        self.lock = threading.Lock()  # 複数スレッドからの同時アクセスを排他制御するロック
        with self.lock:
            try:
    ...
    def close(self):
        """データベース接続を安全に閉じます。アプリ終了時に呼び出してください。"""
        try:
            self.conn.close()
        except Exception:
            pass
